...
module SpreeClient
  class CreateStoreCreditParamsStoreCredit
    ...
    attr_accessor :originator_id

    attr_accessor :originator_type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SpreeClient::CreateStoreCreditParamsStoreCredit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SpreeClient::CreateStoreCreditParamsStoreCredit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'amount_used')
        self.amount_used = attributes[:'amount_used']
      end
      ...
      if attributes.key?(:'originator_id')
        self.originator_id = attributes[:'originator_id']
      end

      if attributes.key?(:'originator_type')
        self.originator_type = attributes[:'originator_type']
      end
    ...
    def hash
      [user_id, category_id, created_by_id, amount, amount_used, memo, currency, amount_authorized, originator_id, originator_type, type_id, store_id, public_metadata, private_metadata].hash
    end
